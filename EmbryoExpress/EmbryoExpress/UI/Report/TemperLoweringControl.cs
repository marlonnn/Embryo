...
namespace EmbryoExpress.UI.Report
{
    public partial class TemperLoweringControl : UserControl
    {
        ...
        {
            this.chartControl1.Titles[0].Text = "Temperature Lower Instrument";
            XYDiagram xyDiagram1 = (XYDiagram)chartControl1.Diagram;

            xyDiagram1.AxisX.Title.Text = "DateTime";
            xyDiagram1.AxisY.Title.Text = this.cmbLoweringItem.SelectedItem.ToString();
        ...
        private void GetStandardPoints(out List<DateTime> xs, out List<int> ys)
        {
            ...
            for (int i = instrumentsList.Count - 1; i >= 0; i--)
            {
                ...
                if (this.cmbLoweringItem.SelectedIndex == 0)
                    ys.Add(instrumentsList[i].TemperLowering.LiquidNitrogen);
                else if (this.cmbLoweringItem.SelectedIndex == 1)
                    ys.Add(instrumentsList[i].TemperLowering.StartTemperature);
                else if (this.cmbLoweringItem.SelectedIndex == 2)
                    ys.Add(instrumentsList[i].TemperLowering.SeedingTemperature);
                else if (this.cmbLoweringItem.SelectedIndex == 3)
                    ys.Add(instrumentsList[i].TemperLowering.EndTemperature);
            }
        }
